...
from shortest_path_distance import shortest_path_distance_ht
...
class embedder:
    def __init__(self, args):
        ...
        if args.im_ratio == 1:
            ...
            idx_train, idx_val, idx_test, class_num_mat,class_num_list,idx_info = utils.split_natural(labels, og_to_new)
            samples_per_label = torch.tensor(class_num_mat[:,0])



        # Manual Setting
        elif args.im_ratio in [ 0.1, 0.05]:#0.2,
            args.criterion = 'max'
            labels, og_to_new = utils.refine_label_order(labels)
            c_train_num = []
            ...
            idx_train, idx_val, idx_test, class_num_mat,class_num_list,idx_info = utils.split_manual(labels, c_train_num, og_to_new)
            samples_per_label = torch.tensor(class_num_mat[:,0])
        
        # LT version
        elif (args.im_ratio == 0.01) & (args.dataset in ['Cora', 'CiteSeer','pubmed']):
                args.criterion = 'mean'
                data = self._dataset
                labels = data.labels
                ...
                #idx_info = utils.get_idx_info(labels, n_cls,data_train_mask)
                ...
                if args.dataset=='Cora': # change label 1<->2 in order to maintain descending order
                    ...
                    labels[labels==1] = 0
                    labels[labels==100] = 1
                elif args.dataset=='pubmed':  # change label 0<->1 in order to maintain descending order
                    labels[labels==0] = 100
                    labels[labels==1] = 0
                    labels[labels==100] = 1
                #'''
                ...
                idx_train, idx_val, idx_test, class_num_mat = utils.split_manual_lt(labels, idx_train, idx_val, idx_test)#val固定25*类别数  test固定55*类别数
                samples_per_label = torch.tensor(class_num_mat[:,0])


        elif (args.im_ratio == 0.2) & (args.dataset in ['computers','photo','cs']):
                args.criterion = 'mean'
                data = self._dataset
                labels = data.labels
                n_cls = labels.max().item() + 1

                train_idx, valid_idx, test_idx, train_node = utils.get_amazon(labels=data.y,\
                                                                       all_idx=[i for i in range(data.x.shape[0])],nclass=n_cls)
                data_train_mask = torch.zeros(data.x.shape[0]).bool().to(args.device)
                data_val_mask = torch.zeros(data.x.shape[0]).bool().to(args.device)
                data_test_mask = torch.zeros(data.x.shape[0]).bool().to(args.device)
                ...
                idx_train, idx_val, idx_test, class_num_mat = utils.split_amazon(labels, idx_train, idx_val, idx_test)
        ...
        if 'lte4g' in args.embedder:
            manual = True if args.im_ratio in [0.1, 0.05] else False#0.2,
            ...
            idx_train_set, degree_dict, degrees, above_head, below_tail = shortest_path_distance_ht(adj,idx_train_set_class,idx_train,labels,
                                                                                                       approximate=5,below=0.8)
            #idx_train_set, degree_dict, degrees, above_head, below_tail = PSD.shortest_path_distance_ht_2(adj,idx_train_set_class,idx_train, labels,
             #                                                                                       approximate = 6, below = 1.5)
