...
class lte4g():
    ...
    def training(self):
        ...
        for seed in range(5, 5+self.args.num_seed):
            ...
            self = embedder(self.args)#load data
            ...
            if self.args.pretrained_encoder:
                ...
                if self.args.rec:
                    model.train()
                    for epoch in range(self.args.ep_pre):#预训练的次数
                        optimizer_fe.zero_grad()
                        optimizer_ep.zero_grad()
                        
                        loss = model(self.features, self.adj, pretrain=True)#调用modeler里的forward
                ...
                for epoch in trange(self.args.ep):
                    ...
                    ori_num = self.labels.shape[0]
                    embed = model.encoder(self.features)
                    embed, labels_new, idx_train_new,adj_up = utils.adasyn_ht(embed, self.class_num_list, self.labels,
                                                                        self.idx_info, self.idx_train,
                                                                        self.adj.to_dense(), ht_dict=self.ht_dict, beta=1,
                                                                        K=5)
                    embed = embed.to(torch.float32).to(self.args.device)
                    generated_G = model.decoder(embed)

                    adj_new = generated_G
                    adj_new = torch.mul(adj_up, adj_new)
                    adj_new[:ori_num, :][:, :ori_num] = self.adj.detach().to_dense()
                    #'''
                    ...
                    if self.args.rec:
                        optimizer_ep.zero_grad()
                        loss_nodeclassification, loss_reconstruction = model(self.features, adj=self.adj, labels=labels_new, idx_train=idx_train_new, embed=embed,weight=class_weight, is_og=True,ori_num=ori_num,adj_new=adj_new)
                        # =======不用过采样时运行下面这行=========
                        ...
                        loss_nodeclassification = model(self.features, adj=adj_new,labels=labels_new, idx_train=idx_train_new, embed=embed, weight=class_weight, is_og=True)
                        # =======不用过采样时运行下面这行=========
                    ...
                    # Evaluation
                    model.eval()
                    #embed = model.encoder(self.features)  #不用过采样时运行下面这两行
                    #output_original = model.classifier_og(embed)
                    output_original = model.classifier_og(embed,adj_new)# =======过采样时运行这行=========
            ...
            for i, label in enumerate(self.labels.unique()):
                ...
                if self.class_num_mat[i,0] < avg_num_candidates:
                    ...
                    if len(candidates_by_neighbors) != 0:
                        ...
                        resources.extend(resource)#原resource + num_candidates_to_fill
            ...
            for sep in ['H', 'T']:
                ...
                if sep == 'H':
                    ...
                    idx_train_sep = self.idx_train_set[sep + 'H']
                    prev_out_local = embed[idx_train_sep]
                    idx_train_new = utils.remove_ht(self.class_num_list, prev_out_local, self.idx_info, idx_train,idx_train_sep, self.labels, self.args.tau,self.ht_dict)
                ...
                for epoch in range(self.args.expert_ep):#专家训练1000epoch
                    #model.train()
                    classifier.train()
                    optimizer.zero_grad()
                    ...
                    #'''
                    # loss = model(self.features, embed=embed, labels=labels_new, adj=adj_new,idx_train=idx_train, classifier=classifier, sep=sep,is_expert=True)
                    #不用下采样时用下面这行
                    #loss = model(self.features, embed=embed, labels=self.labels, adj=self.adj,idx_train=idx_train, classifier=classifier, sep=sep,is_expert=True,adj_new=self.adj)  # 调用modeler里的forward
                    # 下采样用这行
                    loss = model(self.features, labels=self.labels, adj=self.adj,
                                                    idx_train=idx_train_new, classifier=classifier, embed=embed,
                                                    sep=sep, is_expert=True, adj_new=adj_new)
                    loss.backward(retain_graph=True)
                    optimizer.step()
                    ...
                    classifier.eval()
                    #embed = model.encoder(self.features)
                    output = classifier(embed)#GNN_Classifier里的forward gnn+mlp   adj=None的话，adj=self.adj
                    ...
                    if best_metric_expert <= macro_F_val:
                        ...
                        classifier_dict[sep] = deepcopy(classifier) # save best model 当作教师
            ...
            for sep in ['H', 'T']:
                ...
                #'''
                if sep == 'H':
                    idx_train_sep = torch.cat((self.idx_train_set[sep + 'H'], self.idx_train_set[sep + 'T']), 0)
                    prev_out_local = embed[idx_train_sep]
                    idx_train_new = utils.remove_ht(self.class_num_list, prev_out_local, self.idx_info, idx_train,idx_train_sep, self.labels, self.args.tau,self.ht_dict)
                    adj_new = self.adj
                else:
                ...
                for epoch in range(self.args.curriculum_ep):
                    ...
                    optimizer.zero_grad()
                    #下采样用这行
                    kd_head, ce_loss = model(self.features, labels=self.labels,idx_train=idx_train_new, embed=embed,classifier=classifier,
                                             sep=sep,teacher=classifier_dict, is_student=True,adj_new=adj_new)
                    # 不用下采样时用下面这行
                    #kd_head, ce_loss  = model(self.features, embed=embed, labels=self.labels, adj=self.adj, idx_train=idx_train,
                     #            classifier=classifier, sep=sep, teacher=classifier_dict, is_student=True, adj_new=self.adj)  # 调用modeler里的forward

                    # Head-to-Tail Curriculum Learning

                    alpha = utils.scheduler(epoch, self.args.curriculum_ep)  # 调度超参数，随训练进行逐渐变小
                    loss = (1-alpha)*ce_loss + alpha*kd_head
                    #loss = ce_loss + kd_head
                    loss.backward(retain_graph=True)
                    optimizer.step()
...
class modeler(nn.Module):
    ...
    def forward(self, features, adj=None, labels=None, idx_train=None, classifier=None, embed=None, sep=None, teacher=None, pretrain=False, weight=None, is_og=False, is_expert=False, is_student=False,ori_num=None,adj_new=None,generated_G_ori=None):
        if embed == None:
        ...
        if is_og:
            #'''
            output = self.classifier_og(embed, adj_new)  #
            #output = self.classifier_og(embed)  # =======不用过采样时运行这行=========
            #loss_nodeclassfication = F.cross_entropy(output[idx_train], labels[idx_train])
            if self.args.class_weight:#flcal loss
            ...
            else:  # 使用𝛼-balanced变体焦失flacl loss
            ...
            if self.args.rec:
                generated_G = self.decoder(embed)
                loss_reconstruction = utils.adj_mse_loss(generated_G[:ori_num, :][:, :ori_num], adj.detach().to_dense())
        ...
        if is_student:
            ...
            teacher_head_degree = teacher[sep]

            teacher_head_degree.eval()

            out_head_teacher = teacher_head_degree(embed)[idx_train]

            out_head_student = classifier(embed)[idx_train]


            kd_head = F.kl_div(F.log_softmax(out_head_student / self.args.T, dim=1),
                               F.softmax(out_head_teacher / self.args.T, dim=1),
                               reduction='mean') * self.args.T * self.args.T

            if sep in ['T']:
                labels = labels - self.args.sep_point

            ce_loss = F.cross_entropy(classifier(embed, adj_new)[idx_train], labels[idx_train])


            return kd_head, ce_loss